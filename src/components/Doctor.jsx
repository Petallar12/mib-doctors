...
const Doctor = () => {
  ...
  const loadDoctors = async () => {
    try {
      const result = await axios.get(
        "https://backend-doctor.vercel.app/doctors"
      );
  ...
  const applyFilters = () => {
    ...
    if (filters.clinic_name) {
      updatedDoctors = updatedDoctors.filter((doctor) =>
        doctor.clinic_name
          .toLowerCase()
          .includes(filters.clinic_name.toLowerCase())
      );
    }
    ...
    if (name === "name" && value) {
      ...
      if (value === "") {
        const uniqueSpecialties = [
          ...new Set(doctors.map((doctor) => doctor.speciality.toUpperCase())),
        ];
        ...
        const suggestions = [
          ...new Set(
            doctors
              .filter((doctor) =>
                doctor.speciality.toLowerCase().startsWith(value.toLowerCase())
              )
      ...
      if (value === "") {
        const uniqueClinics = [
          ...new Set(doctors.map((doctor) => doctor.clinic_name)),
        ];
        ...
        const suggestions = [
          ...new Set(
            doctors
              .filter((doctor) =>
                doctor.clinic_name.toLowerCase().startsWith(value.toLowerCase())
              )
  ...
  const handleClinicInputClick = () => {
    if (filters.clinic_name === "") {
      const uniqueClinics = [
        ...new Set(doctors.map((doctor) => doctor.clinic_name.toUpperCase())),
      ];
      setClinicNameSuggestions(uniqueClinics);
    }
  };
              ...
              <ul className="suggestions">
                {specialitySuggestions.map((suggestion, index) => (
                  <li
                    key={index}
                    onClick={() =>
                      handleSuggestionClick(suggestion, "speciality")
                    }
                  >
                    {suggestion}
                  </li>
              ...
              <ul className="suggestions">
                {clinicNameSuggestions.map((suggestion, index) => (
                  <li
                    key={index}
                    onClick={() =>
                      handleSuggestionClick(suggestion, "clinic_name")
                    }
                  >
                    {suggestion}
                  </li>
          ...
          <div className="doctor-box" key={doctor.id}>
            <div className="photo-box">
              <img
                src={
                  doctor.image_url
                    ? `${process.env.PUBLIC_URL}/images/${doctor.image_url}`
                    : placeholderImage
                }
            ...
            <h3>
              <Link to={`/doctor/${doctor.id}?page=${currentPage}`}>
                {doctor.name.toUpperCase()}
              </Link>
            </h3>
            {doctor.speciality && (
              <p>
                <FaUserMd /> {doctor.speciality}
              </p>
            )}
            {doctor.clinic_name && (
              <p>
                <FaClinicMedical /> {doctor.clinic_name}
              </p>
            )}
          </div>
        ))}
